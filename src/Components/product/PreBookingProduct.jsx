...
const Product = () => {
  ...
  const productStats = useMemo(() => {
    return {
      total: allProduct.filter((p) => Boolean(p?.preBooking)).length,
      active: allProduct.filter((p) => Boolean(p?.preBooking) && Number(p.stock) > 0).length,
      inactive: allProduct.filter((p) => Boolean(p?.preBooking) && Number(p.stock) <= 0).length,
      hotSell: allProduct.filter((p) => Boolean(p?.preBooking) && Boolean(p.hotSell)).length,
  ...
  const filteredProducts = useMemo(() => {
    ...
    let result = allProduct.filter((product) => Boolean(product?.preBooking));
    ...
    if (filterStatus === "preBooking") {
      // Base query is already restricted to pre-booking products.
  ...
  const exportProducts = async () => {
    ...
    try {
      ...
      link.download = `darsh-prebooking-products-${
        ...
        <div className="mb-7 flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 pl-4 text-sm text-zinc-500">
              {filteredProducts.length} of {productStats.total} pre-booking sarees shown
            </p>
          </div>
